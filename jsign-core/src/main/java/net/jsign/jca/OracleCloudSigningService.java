...
public class OracleCloudSigningService implements SigningService {
    ...
    public List<String> aliases() throws KeyStoreException {
        ...
        try {
            ...
            for (Map<String, ?> vault : (Map<String, ?>[]) result.get("result")) {
                if ("ACTIVE".equals(vault.get("lifecycleState"))) {
                    ...
                    Map<String, ?> result2 = managementClient.get("/20180608/keys?compartmentId=" + credentials.getTenancy());
                    for (Map<String, ?> key : (Map<String, ?>[]) result2.get("result")) {
