...
public class NAVXSignerTest {
    ...
    public void testSign() throws Exception {
        ...
        File targetFile = new File("target/test-classes/minimal-signed.navx");
    ...
    public void testSignTwice() throws Exception {
        File sourceFile = new File("target/test-classes/minimal.navx");
        File targetFile = new File("target/test-classes/minimal-signed-twice.navx");
    ...
    public void testSignThreeTimes() throws Exception {
        File sourceFile = new File("target/test-classes/minimal.navx");
        File targetFile = new File("target/test-classes/minimal-signed-three-times.navx");
    ...
    public void testReplaceSignature() throws Exception {
        File sourceFile = new File("target/test-classes/minimal.navx");
        File targetFile = new File("target/test-classes/minimal-re-signed.navx");
    ...
    public void testSignInMemory() throws Exception {
        ...
        try (Signable file = new NAVXFile(channel)) {
            signer.sign(file);
            data = Arrays.copyOf(channel.array(), (int) channel.size());
        }

        try (Signable file = new NAVXFile(new SeekableInMemoryByteChannel(data))) {
            SignatureAssert.assertSigned(file, SHA256);
        }
    }
}
